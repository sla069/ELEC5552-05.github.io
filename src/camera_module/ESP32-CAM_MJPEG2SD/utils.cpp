...
char AP_SSID[MAX_HOST_LEN] = "Drone Live Footage";
...
static void setWifiAP() {
  if (!APstarted) {
            // Force your custom SSID here
        const char* customSSID = "Drone Live Footage";
        const char* customPass = "";
    ...
    WiFi.softAP(customSSID, AP_Pass);
    strcpy(AP_SSID, customSSID);          // update the global for logging

    LOG_INF("Custom AP SSID applied: %s", AP_SSID);
